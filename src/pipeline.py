...
def newest_file(dir_path: Path, pattern: str = "*.json") -> Optional[Path]:
...
def parse_integrity_summary(evidence_path: Path) -> dict[str, Any]:
    """
    Your integrity summary JSON previously looked like:
      bad_rows_total, bad_rows_null_code, bad_rows_code_not_in_countries, unique_bad_codes_sample, ...
    We extract key ones and compute has_bad_rows.
    """
    data = read_json(evidence_path)
    ...
    out["has_bad_rows"] = (data.get("bad_rows_total", 0) or 0) > 0
    return out
...
def main() -> int:
    ...
    # Integrity evidence: your check_integrity.py may write JSON into repo_root/reports OR repo_root/src/reports
    integrity_hints = [
        root / "reports",
        root / "src" / "reports",
    ]
    ...
    steps: list[tuple[str, list[str], Optional[list[Path]], Any]] = [
        ...
        (
            "03_check_integrity",
            [args.python, str(src_dir / "check_integrity.py")],
            integrity_hints,
            parse_integrity_summary,
        ),
        (
            "04_split_quarantine",
            [args.python, str(src_dir / "split_quarantine.py")],
            None,
            None,
        ),
    ]
    ...
    if not args.skip_load:
        steps.append(
            (
                "05_load_to_postgres",
                [args.python, str(src_dir / "load_to_postgres.py")],
                None,
                None,
            )
        )
    ...
    for name, cmd, evidence_hints, parser_fn in steps:
        ...
        # - Integrity evidence might report bad rows even if script exits 0
        if name == "03_check_integrity":
            if r.parsed_metrics and r.parsed_metrics.get("has_bad_rows") is True:
                if args.strict:
                    r.ok = False
                    r.error = r.error or "Integrity check found bad rows (bad_rows_total > 0)."
